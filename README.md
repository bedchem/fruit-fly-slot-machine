...
- **Live hashtag feed:** the panel also shows TikTok's official hashtag embed for `#sabrinacarpenter` (`EDIT_TAG` in `src/game/tiktokEdits.js`): whatever TikTok lists under the tag right now, fetched fresh on every visit, not a fixed list. It is TikTok's own widget, so TikTok picks and orders the posts; the page cannot see which ones, which is why the flies' phones still play the listed edits one by one. It is loaded as TikTok's embed iframe (`https://www.tiktok.com/embed/tag/{tag}`) without TikTok's `embed.js`, and sizes itself from the height the frame reports. An empty `EDIT_TAG` leaves it out.
