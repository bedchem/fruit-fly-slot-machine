...
export default function ScrollApp() {
  ...
  const [ui, setUi] = useState({ thread: [], morning: null, toast: null });
  ...
  const onTick = useCallback(() => {
    ...
    setUi((prev) => (
      prev.beat === beat && prev.threadLen === d.thread.length && prev.morning === d.morning && prev.toast === toast.current
        ? prev
        : { beat, thread: d.thread.slice(), threadLen: d.thread.length, morning: d.morning, toast: toast.current }
    ));
  }, []);
  ...
  return (
    <div className="app venue-dark venue-scroll">
      ...
      <aside className="panel">
        <Phones duoRef={duoRef} canvasRefs={canvasRefs} />
        <Feeds duoRef={duoRef} />
