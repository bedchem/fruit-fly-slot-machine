...
import { Bar, MM_PER_PERMILLE } from '../src/game/bar.js';
...
const b = new Bar({
  ...
  onEvent: (type, d) => {
    ...
    const body = `EtOH ${b.permille.toFixed(2).padStart(5)} ‰  nic ${b.nicotine.toFixed(1).padStart(5)}  hang ${b.hangover.toFixed(2)}`
...
console.log(`\n${minutes} real minutes (${((performance.now() - started) / 1000).toFixed(1)}s to run): `
  ...
  + `all-time peak ${(b.allTimePeak / MM_PER_PERMILLE).toFixed(2)} ‰, worst hangover ${b.hangoverWorst.toFixed(2)}`);
